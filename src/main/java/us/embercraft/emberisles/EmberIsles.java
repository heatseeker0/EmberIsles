...
public class EmberIsles extends JavaPlugin {
    ...
    public void applyConfig() {
        ...
        for (String key : config.getConfigurationSection("party-settings.ranks").getKeys(false)) {
            partyDefinitions.addPartyRank(config.getString(String.format("party-settings.ranks.%s.permission", key)), config.getInt(String.format("party-settings.ranks.%s.party-limit", key)));
        }
        ...
        for (WorldType type : WorldType.values()) {
            ...
            for (String schemKey : config.getConfigurationSection(String.format("schematics.%s", type.getConfigKey())).getKeys(false)) {
                ...
                SchematicDefinition definition = new SchematicDefinition(type, material, (short) config.getInt(String.format("schematics.%s.%s.durability", type.getConfigKey(), schemKey)), MessageUtils.parseColors(config.getString(String.format("schematics.%s.%s.title", type.getConfigKey(), schemKey), "Undefined")), config.getString(String.format("schematics.%s.%s.permission", type.getConfigKey(), schemKey)), schemFile, MessageUtils.parseColors(config.getStringList(String.format("schematics.%s.%s.lore", type.getConfigKey(), schemKey))), MessageUtils.parseColors(config.getStringList(String.format("schematics.%s.%s.noperm-lore", type.getConfigKey(), schemKey))), homeBlockMaterial);
    ...
    public void createIsland(final Player player, FutureMenuCommand cmd) {
         ...
         */
        final Location pasteLoc = getWorldManager().gridToWorldCoordA(cmd.getWorldType(), key.getGridX(), key.getGridZ()).add(getWorldManager().getDefaultWorldSettings(cmd.getWorldType()).getIslandSize() >> 1, getWorldManager().getDefaultWorldSettings(cmd.getWorldType()).getY(), getWorldManager().getDefaultWorldSettings(cmd.getWorldType()).getIslandSize() >> 1);
